...
const ProductEditPage: React.FC<ProductEditPageProps> = ({
  ...
  const fetchData = async () => {
    try {
      ...
      let variantAttributes: Array<{ name: string; value: string }> = [];
      if (product.productVariant) {
        try {
          const attrs = await getVariantAttributesByVariantId(
            product.productVariant.id,
          );
          variantAttributes = attrs.map((attr: any) => ({
            name: attr.name,
            value: attr.value,
          }));
        } catch (err) {
      ...
      setFormData({
        ...
        variant: product.productVariant
          ? {
              ...
              salePrice: product.productVariant.salePrice || 0,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updateProduct(Number(productId), requestData);
        ...
        <div className="mx-auto max-w-4xl">
          <div className="mb-8">
            ...
            <p className="mt-1 text-sm text-gray-600">
              Cập nhật thông tin sản phẩm mỹ phẩm
            </p>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                onClick={() => onNavigate('/admin/products')}
                disabled={saving}
                className="flex-1 rounded-xl border-2 border-gray-300 bg-white px-6 py-4 font-semibold text-gray-700 shadow-lg transition-all hover:bg-gray-50 disabled:bg-gray-100"
              >
                Hủy
              </button>
            </div>
